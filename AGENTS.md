...
- `skills/design-project-architecture/references/` is maintained in this repository. It contains Cheng's technical profile, architecture decision guidance, reusable package and project patterns, GitHub Actions and static-site SEO delivery guidance, frontend ESLint defaults, frontend theme-system, browser-theme lifecycle, website UI icon guidance, PWA icon guidance, the maintained blue theme preset, and relative provenance labels distilled from a private article corpus and maintained project guides. Do not add machine-specific source paths or treat historical articles as current technical authority.
