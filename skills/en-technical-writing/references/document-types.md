...
| `AGENTS.md`           | Give coding agents accurate repository navigation and operating constraints | Current instructions, relevant code and configuration, execution flow, validation, and change rules | Preserve an existing useful structure; when creating, cover only applicable verified facts and link extracted guides |
...
| `AGENTS.md`           | Only for commands or structures agents must use | Required                                     | Stale repository claims; invented architecture; excessive detail | Only when the repository uses React | No                           | No                           |
...
Treat `AGENTS.md` as operational guidance and repository navigation for coding agents, not as a general project introduction.

When updating an existing file:

- Read the current file and inspect the repository before changing it.
- Preserve a clear existing hierarchy, section order, terminology, and level of detail.
- Correct inaccurate statements, add information to the relevant existing section, and remove obsolete guidance.
- Ground architecture, execution, configuration, data-flow, and validation statements in current code or configuration.

When creating a file, include only applicable repository facts: purpose, key directories, entry points, startup or execution flow, configuration, build, test, and validation pipeline, data flow, and important change rules. Describe frontend component hierarchy, state, prop flow, context, and rendering boundaries only when they exist. Describe backend routing, controllers, services, repositories, models, and request or data flow only when the repository implements them. Do not invent conventional layers or directories.

Target approximately 300 lines when practical. Treat approximately 500 lines as a strong signal to extract detailed material, not as a limit. Move a detailed topic to an existing or new `guides/<topic>.md`, keep a concise summary and direct link in `AGENTS.md`, and state when agents should read the guide. Use a nested `AGENTS.md` only when its instructions apply to a narrower directory scope.
