...
2. Extract only supplied facts. Preserve exact pet names, grammatical gender, dates, places, BGM titles, health facts, campaign tags, and version identifiers.
...
7. Draft each language naturally from the same fact set. Adapt rather than translate literally, but keep the event, names, places, dates, BGM, health status, and fictional status consistent.
...
- Do not add BGM, pet names, copyright text, platform tags, or medical detail without support or a direct request.
