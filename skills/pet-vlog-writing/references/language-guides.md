...
- Chinese tags usually use familiar concepts such as `#猫咪`, `#小猫`, `#猫咪日常`, plus scene-specific tags. Do not pad every set with all recurring generic tags.
- When compact tags are requested, output `#猫咪#小猫#猫咪日常` with no spaces.
- Vary overused phrases including `治愈系日常`, `乖巧小天使`, `温柔的陪伴`, and `小猫的治愈睡颜`.
...
- Do not reuse awkward source phrases such as `優しい猫の仲間`; adapt the intended meaning naturally, for example `やさしく寄り添う猫`.
- For confirmed female Dudu, avoid masculine self-reference such as `ボク` unless the user requests a character voice.
- Japanese tags should use natural labels such as `#猫`, `#子猫`, `#猫のいる暮らし`, and scene-specific terms. Mix English only for established platform tags, BGM/proper names, or deliberate title styling.
- A Japanese title may stand alone when requested; do not invent a description merely to mirror other languages.
