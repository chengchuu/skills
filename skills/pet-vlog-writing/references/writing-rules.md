...
- Avoid absolute superlatives by default. Source titles such as “world's most healing” are evidence of occasional intensity, not a reusable factual claim.

## Category-specific tendencies

- Sleep and relaxation: foreground posture, eyes, blanket, light, sofa, or the transition into sleep; keep rhythm slow.
- Companionship: use proximity and small gestures. Avoid claiming love or trust unless supplied.
- Playful and funny: use contrast, a question, mock-serious framing, or a quick punchline; do not fabricate a mishap.
...
- Use one emoji or a small emoji pair for a warm social caption when requested or platform-appropriate.
- Use a colon, question, or mixed-language title for contrast or platform rhythm.
- Use owner-facing first person in companionship clips when the relationship is supplied.
- Pair broad discovery tags with scene-specific tags. Typical sets contain about 4–10 tags, though source examples vary.
- Add a second compact hashtag line only when requested or when reproducing a known platform-ready schema.

## Rare or experimental patterns

- Song-title callbacks, dramatic fantasy narration, giant-cat city scenes, costume concepts, and mixed-language titles are category-specific.
- Questions that speculate playfully (`发现了什么秘密？`) may be used only when clearly rhetorical and not presented as fact.
- Campaign tags such as `#AiScReam` are preserved only when supplied.
