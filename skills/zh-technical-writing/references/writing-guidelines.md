...
一级标题中的代码块应当使用反引号而不是半角双引号，但是其他标题和内容不受此规则影响。
...
- 调整可编辑 Markdown 的格式时，可以将现有纯文本代码块的 `text` 信息字符串改为 `plain`，但不得修改代码块中的内容。
- 用户或目标仓库明确要求其他信息字符串时，保留其要求。
- 对于仅审阅任务，只报告不符合此约定的纯文本代码块，不要改写原文。

## 正文换行

- 普通正文遵循“一段一行”，不要按固定行宽在同一段内强制换行。显示宽度由编辑器的软换行功能处理，软换行不属于 Markdown 源码换行。
- 仅在 Markdown 语义需要时换行，例如标题、段落边界、列表、表格、引用和代码块。
- 不要使用 `<br>`、行尾双空格或单个源码换行实现视觉排版，除非用户或目标仓库明确要求。
- 改写、翻译或校对现有内容时，保留原文、代码、引用和其他受保护内容中已有的必要换行；不要以“一段一行”为由破坏其结构或语义。

## 表格

写作、改写、翻译或校对可编辑的 Markdown 文档时，默认将所有表格列设为左对齐，并在每个分隔单元格的开头添加冒号。

- 根据标题和数据单元格中的最长内容，将同一列规范为一致的显示宽度。使用半角空格补齐较短的单元格，使竖线在源码中垂直对齐；不要使用制表符。
- 按显示宽度计算，不要直接使用 Unicode 字符数量。ASCII 和其他半角字符按 1 个显示宽度计算；中日韩表意文字、假名、韩文音节和全角字符按 2 个显示宽度计算；组合附加符号按 0 个显示宽度计算。例如，`App` 的显示宽度为 3，`应用` 的显示宽度为 4。
- 对应的分隔单元格必须与该列补齐后的单元格使用相同显示宽度，同时保留开头的冒号和 Markdown 所需的最少连字符。
...
- 用户或目标仓库明确要求其他对齐方式时，保留其要求。
- 不要修改代码块、引用块、生成文件或受保护源内容中的表格。
- 对于仅审阅任务，只报告显示宽度或分隔行对齐不一致的表格，不要改写表格或文件。
...
逗号分割的长句，总长度不应该超过 100 字。不要根据编辑器显示行数或 Markdown 源码行数判断句长。
...
- 段落长度应根据主题、句子数量和阅读负担判断，不要根据编辑器显示行数或 Markdown 源码行数判断。内容过长时，拆分为新的语义段落，不要在同一段内插入硬换行。
...
(1) 中文语句的标点符号，除了引号、括号、冒号之外，其他均应该采取全角符号，这样可以与全角文字保持视觉的一致。
...
(1) 中文正文使用半角直双引号 (`" "`)，不使用中文弯引号 `“ ”`。标题、列表、表格说明、按钮名称和普通强调语中的引号也遵循此规则。双引号紧贴内部文字，不在引号内侧添加空格。
...
(2) 嵌套引用的外层使用半角双引号，内层使用半角单引号 (`' '`)。每层的左右引号使用相同的 ASCII 字符，不使用弯单引号。

```
例句: 鲍勃解释道: "我要放音乐，可萨利说，'不行！'。"
```

(3) 代码块、行内代码、命令、URL、标识符和要求原样保留的引用不做标点替换。一级标题中的代码标识符继续使用反引号，不改成双引号。标点字符本身作为讲解对象时，也应保留。

## 括号

(1) 补充说明时，使用半角圆括号 (`()`)，括号前后有文字的话加空格，没有的话不加。
...
(1) 中文叙述和标题中的冒号统一使用半角 `:`，不使用全角 `：`。冒号后同一行还有文字时，加一个半角空格；位于行尾时，不添加尾随空格。
...
(3) 时间、URL 和代码中的冒号遵循各自语法，不套用正文的空格规则。已有代码、命令和其他受保护内容保持原样。
